...
console.log(`\nFor a local .env file — paste this line exactly as-is (the backslashes are required):`);
console.log(`${label.toUpperCase()}_AUTH_PASS_HASH="${escapedHash}"`);
console.log(`\nFor Vercel's dashboard (Settings > Environment Variables) — paste the RAW value, no backslashes:`);
console.log(hash);
console.log();
